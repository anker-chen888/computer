...
bool RtmpConnection::OnRead(BufferReader &buffer)
{
    ...
    {
        ret = HandleChunk(buffer); // 解析Chunk
    }
    // 如果握手未完成，解析握手数据进行握手
    else
    {
        ...
        if(handshake_->IsCompleted()) // 握手成功
        {
...
bool RtmpConnection::HandleMessage(RtmpMessage &rtmp_msg)
{
    ...
    case RTMP_NOTIFY:   // 通知
        ...
        break;
    case RTMP_SET_CHUNK_SIZE: // 设置Chunk大小
...
{
    uint8_t type = RTMP_VIDEO; // 初始化视频类型为RTMP_VIDEO
    uint8_t* playload = (uint8_t*)rtmp_msg.playload.get(); // 获取视频数据的负载指针
    uint32_t lenght = rtmp_msg.lenght; // 长度
...
bool RtmpConnection::HandleCreateStream()
{
    ...
    int stream_id = rtmp_chunk_->GetStreamId(); // 获取流ID
    ...
    // 分配一个唯一的流ID
    SendInvokeMessage(RTMP_CHUNK_INVOKE_ID,amf_encoder_.data(),amf_encoder_.size()); // 发送消息
    stream_id_ = stream_id; // 更新流ID
    return true;
}
...
bool RtmpConnection::SendMediaData(uint8_t type, uint64_t timestamp, std::shared_ptr<char> playload, uint32_t playload_size)
{
    ...
    is_playing_ = true; // 标记当前连接为正在播放状态
    ...
    // 构造RTMP消息
    RtmpMessage rtmp_msg;
    rtmp_msg._timestamp = timestamp; // 时间戳
    rtmp_msg.stream_id = stream_id_; // 流ID
    rtmp_msg.playload = playload;   // 负载数据
    rtmp_msg.lenght = playload_size; // 大小

    //还有msg类型
    if(type == RTMP_VIDEO ||type ==RTMP_AVC_SEQUENCE_HEADER)
    {
        rtmp_msg.type_id = RTMP_VIDEO;
        SendRtmpChunks(RTMP_CHUNK_VIDEO_ID,rtmp_msg); // 转为Chunk然后进行发送
    }
