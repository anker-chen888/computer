...
{
    while(buffer.ReadableBytes() > 0) // 读取数据
    {
...
void SigConnection::HandleMessage(BufferReader &buffer)
{
    ...
    packet_head* data = (packet_head*)buffer.Peek(); // 解析信令包头
...
void SigConnection::Clear()
{
    ...
    for(auto iter : objectes_)
    {
        ...
        if(con)
        {
            ...
            con->Send((const char*)&body,body.len); // 发送出去
        }
    }
    objectes_.clear();
...
// 处理客户单发送的加入房间请求，根据当前连接的状态和系统中的其他连接情况，决定是否允许客户端加入房间，并返回相应的应答
void SigConnection::HandleJion(const packet_head *data)
{
    ...
    if(this->IsNoJion())
    {
        std::string code = body->GetId(); // 标识Id由客户端指定
        TcpConnection:Ptr ptr = ConnectionManager::GetInstance()->QueryConn(code);  //标识符是否已经存在
        // 已经存在
        if(ptr)
        {
            reply_body.SetCode(ERROR);
            this->Send((const char*)&reply_body,reply_body.len); // 发送错误数据给客户端
            return;
        }
        code_ = code;
        state_ = IDLE;
        ConnectionManager::GetInstance()->AddConn(code, shared_from_this()); // 将当前连接添加到连接管理器
        printf("Jion count: %d\n",ConnectionManager::GetInstance()->Size());
        reply_body.SetCode(SUCCESSFUL);
        this->Send((const char*)&reply_body,reply_body.len);    // 发送成功应答包
        return;
    }
    //已经创建
    reply_body.SetCode(ERROR);
...
// 用于处理客户端获取流的请求。
// 它会根据目标客户端的连接状态，决定是否允许当前客户端获取流，并执行相应的操作。
void SigConnection::DoObtainStream(const packet_head *data)
{
    ObtainStreamReply_body reply;   // 发送给客户端的应答包
    CreateStream_body create_reply; // 通知目标客户端创建流
    std::string code = ((ObtainStream_body*)data)->GetId(); // 要获取流的客户端Id
    ...
    if(this->IsIdle())//本身是空闲就去获取流
    {
        ...
        case IDLE:
            ...
            this->Send((const char*)&reply,reply.len); // 发送控制端包
            break;
        case NONE:
            printf("目标没上线\n");
            reply.SetCode(ERROR);
...
void SigConnection::DoCreateStream(const packet_head *data)
{
    ...
    for(auto idefy : objectes_)
    {
        ...
        case PUSHER:
            ...
            con->Send((const char*)&body,body.len); // 发送错误
            break;
        case PULLER:
            this->state_ = PUSHER;  // 更新连接状态为推流
