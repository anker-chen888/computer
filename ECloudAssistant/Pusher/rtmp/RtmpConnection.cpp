...
bool RtmpConnection::OnRead(BufferReader &buffer)
{
    ...
    else
    {
        ...
        int res_size = handshake_->Parse(buffer,res.get(),4096); // 握手
        if(res_size < 0)
        {
            ret = false;
        }
        if(res_size > 0)
        {
            this->Send(res.get(),res_size); // 发送握手数据
        }
...
bool RtmpConnection::HandleChunk(BufferReader &buffer)
{
    ...
    do
    {
        ...
        ret = rtmp_chunk_->Parse(buffer,rtmp_msg); // 解析得到rtmp_msg数据
...
bool RtmpConnection::HandleMessage(RtmpMessage &rtmp_msg) //INVOKE CHUNK_SIZE
{
    ...
    {
    case RTMP_INVOKE:
        ret = HandleInvoke(rtmp_msg); // 执行各种控制操作
        break;
    case RTMP_SET_CHUNK_SIZE: // 设置块大小
...
bool RtmpConnection::HandleInvoke(RtmpMessage &rtmp_msg)
{
    ...
    if(method == "_result")  // 服务端执行后返回的结果
    {
        ret = HandleResult(rtmp_msg);
    }
    else if(method == "onStatus") // 处理服务器状态的通知
    {
...
bool RtmpConnection::Publish()
{
    ...
    SendInvokeMsg(RTMP_CHUNK_INVOKE_ID,amf_encoder_->data(),amf_encoder_->size()); // 发送命令
    return true;
}

// 构造并发送一个 DeleteStream 命令，通知服务器删除当前的推流
...
// 处理RTMP协议中的onStatus消息
// onStatus 消息通常用于通知客户端服务器端的状态变化，例如推流或删除流的结果
...
// 将一个RTMP消息分割成多个块，然后存储到Bufferwrite里面就好，后面调用它的send函数(实际是::send函数)
