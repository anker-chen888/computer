...
int WASAPICapture::start()
{
    ...
    HRESULT hr = m_audioClient->Start();    // 启动音频客户端
    if(FAILED(hr))
    {
...
int WASAPICapture::capture()
{
    ...
    while (packetLenght > 0) {
        hr = m_audioCaptureClient->GetBuffer(&pData,&numFrameAvailabel,&flags,NULL,NULL); // 获取音频数据大小
        if(FAILED(hr))
        {
